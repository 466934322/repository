...
function __goods_catalog_del(id, callback)
{
    if (!confirm('确认删除吗？其下的产品会变成无分类状态！')) return;
    $.get('admin.php?mod=catalog&code=goodscate_del&op=ajax&id='+id.toString()+$.rnd.stamp(), function(data){
        if (data == 'ok')
        {
            callback(id);
        }
