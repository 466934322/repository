...
class ModuleObject extends MasterObject
{
	...
	public function main(){
		...
		$product = logic('product')->GetList(logic('misc')->City('id'), NULL, '`is_countdown`=1 AND (p.overtime > '.time().' or p.overtime < 1)');
		...
		if($product){
			foreach($product as &$v){
				...
				$v['overtime'] = $v['overtime']?date('Y-m-d H:i:s', $v['overtime']):'';
			}
		}
		$this->Title = "限时抢购";
		include handler('template')->file('@m/buy_countdown');
	}
}
