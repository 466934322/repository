...
class ModuleObject extends MasterObject
{
	...
	function Login()
	{
		...
		{
			$this->Messager("您已经使用用户名 " . MEMBER_NAME . " 登录系统，无需再次登录！", $this->Config['site_url'] . '/m.php?');
		}
		...
		include (template("@m/account_login"));
	}
	...
	function Login_done()
	{
		...
		$ref = account()->loginReferer();
		$ref || $ref = $this->Config['site_url'] . '/m.php?';
		$this->Messager(__('登录成功！').$loginR['result'], $ref);
	}
	...
	function Logout()
	{
		...
		$this->Messager($logoutR['result'] . '退出成功', $this->Config['site_url'] . '/m.php?');
	}
	...
	function Register_phone() {
		...
		include(template('@m/account_register_phone'));
	}
	...
	function Register()
	{
		$this->Title = __('注册');
				$city = logic('misc')->CityList();
		$action = 'm.php?mod=account&code=register&op=done';
		$home_uid = $_GET['u'];
		if(false!=ini('member.register.closemail')) {
			$this->Messager(null, 'm.php?mod=account&code=register&op=phone&u='.(int)$_GET['u']);
		}
		include template("@m/account_register");
	}

	function Register_done()
	{
		if(false!=ini('member.register.closemail')) {
			$this->Messager('已经关闭了邮箱注册功能，请使用手机号进行注册', 'm.php?mod=account&code=register&op=phone&u='.(int) $_POST['home_uid']);
		}
	...
	public function quickSignIn() {
		...
		$signInType = get('signInType', 'string');					$signInPhone = get('signInPhone', 'string');				$signInDynamicCode = get('signInDynamicCode', 'string');					$signInPassword = get('signInPassword', 'string');					
				if (!isset($signInType) || !in_array($signInType, array('password', 'dynamicCode'))) {
			...
			$this->quickLogin($signInPhone, $signInPassword);
				...
				if($result) {
					exit($result);
								...
								$rets = logic('account')->RegisterPhone($signInPhone, $signInDynamicCode, $signInDynamicCode, 0, false);
