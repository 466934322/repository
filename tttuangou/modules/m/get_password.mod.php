...
class ModuleObject extends MasterObject
{
	...
	function Main()
	{
		...
		$seccode = ($_GET['seccode'] ? $_GET['seccode'] : $_POST['seccode']);
		...
		if ('step1' == $act) {
			...
			if('phone' == $by) {
				if(false == logic('seccode')->verify($seccode)) {
		...
		include(template('@m/get_password_main'));
	}
	...
	function _resetCheck()
	{
		...
		if($dateline < $timestamp - 86400 || $operation != 1 || $idstring != $id)
		{
			$message=array(
				...
				"如您想重新设置密码，请<a href='m.php?mod=get_password'>单击此处</a>。"
			);
			$this->Messager($message,0);
		}
		$member['id'] = $id;
