...
class ModuleObject extends MasterObject
{
	public function __construct($config)
	{
		...
		if (true == in_array(ini('settings.site_domain'), array('localx.uuland.org', 'dev.tttuangou.net', 'tuangou.com', )))
		{
	...
	public function DBS_save()
	{
		...
		ini('settings.db_table_prefix', ($db['prefix'] != '') ? $db['prefix'] : 'tttuangou_'.$this->RandString(6).'_');
				header('Location: inizd.php?mod=install&code=config');
	}
	...
	public function Config_save()
	{
		...
		header('Location: inizd.php?mod=install&code=install');
	}
	...
	{
		$c = ini('__install_config_temp');
		ini('settings.api_version', '0');
		ini('settings.wap_version', '0');
	...
	private function FunctionTest($list)
	{
		...
		foreach ($list as $i => $func)
		{
			...
			elseif ('iconv' == $func) {
				$return[] = array(
						'name' => $func,
						'sp' => (function_exists('iconv') || function_exists('mb_convert_encoding'))
					);
			}
			else
			{
