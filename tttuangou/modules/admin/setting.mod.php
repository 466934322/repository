...
class ModuleObject extends MasterObject
{
	...
	function ModifyRewrite()
	{
		...
		include($config_file);
				...
				$mode_select=handler('form')->Select('mode',$mode_list,$_rewrite['mode']);
	...
	function ModifyFilter()
	{
		...
		$enable_radio=handler('form')->YesNoRadio("filter[enable]",(int)$filter['enable']);
...
# END Tttuangou";
		...
		@$len = handler('io')->WriteFile(ROOT_PATH . ".htaccess", $str);
		...
		foreach ($_dirs as $_dir) {
			...
			$str="# BEGIN Tttuangou dir safe protect for {$_dir}
<IfModule mod_rewrite.c>
RewriteEngine On
".
			($is_local?"Options FollowSymLinks":"")
...
# END Tttuangou ".date("Y-m-d H:i:s");

			@$len = handler('io')->WriteFile($_path . '.htaccess', $str);
