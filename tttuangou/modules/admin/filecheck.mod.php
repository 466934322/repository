...
class ModuleObject extends MasterObject
{
	...
	public function checkout()
	{
		...
		foreach ($showlist as $file => $md5)
		{
			...
			if ($statusf)
			{
				$filelist[$dir][] = array('file' => basename($file), 'size' => file_exists(ROOT_PATH.$file) ? round(filesize(ROOT_PATH.$file)).' Bytes' : '', 'filemtime' => $filemtime, 'status' => $status, 'bom' => $this->boms[$file]);
			}
		}
