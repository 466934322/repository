...
class ModuleObject extends MasterObject
{
	...
	function Main()
	{
		...
		$turnon_radio=handler('form')->YesNoRadio('config[turnon]',(int)$config['turnon'],'','class="radio"');
			...
			while ($row=$query->GetRow())
			{
				...
				if($this->ad)
				{
					$show_ad=isset($config['list'][$row['name']]['show_ad'])
							...
							1;
					$row['show_ad_radio']=handler('form')->YesNoRadio("config[list][{$row['name']}][show_ad]",$show_ad,'',"class='radio'");
				}
				$row['today_times']=0;
				$name_list[]=$row['name'];
