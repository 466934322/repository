...
class Order_Parser_CallbackLogic extends CallbackLogic
{
        	...
        	if(is_array($orders) && count($orders)) {
        		foreach($orders as $order) {
        			$status = $status_default;
        			        			if(in_array($status, array('WAIT_SELLER_SEND_GOODS','__CREATE__')) &&
        				($product = logic('product')->SrcOne($order['productid'])) && 
        				'ticket' == $product['type']) {
        				$status = 'TRADE_FINISHED';
        			} 
					
										if (in_array($status, array('WAIT_SELLER_SEND_GOODS', 'FINISHED')) && logic('yungou')->isYunGouOrder($order['orderid'])) {						
						$this->parse_yunGouPayment($payment, $order, $status);
					} else {						
	...
	function Parse_WAIT_SELLER_SEND_GOODS($payment = false,$order = array())
	{
		...
		if (logic('yungou')->isYunGouOrder($trade['sign'])) {
			$this->parse_yunGouPayment($payment, $order, 'WAIT_SELLER_SEND_GOODS');
			return ;
		}
	...
	function Parse_TRADE_FINISHED($payment = false,$order = array())
	{
				$trade = logic('pay')->TradeData($payment,$order);
		if (logic('yungou')->isYunGouOrder($trade['sign'])) {
			$this->parse_yunGouPayment($payment, $order, 'TRADE_FINISHED');
			return ;
		}
	...
	public function parse_yunGouPayment($payment = false,$order = array(), $sourceStatus = '') 
	{
				$trade = logic('pay')->TradeData($payment,$order);
		$order = logic('order')->_TMP_Payed($trade, $payment);			
		$error = false;																if (isset($order['false'])) {
			$error = $order['false'];
		}
		...
		if (!$error)
		{
						...
						if('WAIT_RESULT' != $order['process']) { 				 
				 				 for ($i = 0; $i < $order['productnum']; $i++) {						 
					 logic('yungou')->addCanYuJiLu($order['productid'], $order['orderid'], $order['userid'], $i);
					 
				 }

				 logic('order')->baseUpdate(array('process' => 'WAIT_RESULT'), 'orderid = "' . $trade['sign'] . '"');
			}
			 
			 			 $sellsCount = logic('product')->SellsCount($order['productid']);
			 logic('product')->Update_direct($order['productid'], array('sells_count' => $sellsCount));
			 
			 			 if (logic('yungou')->isYunGouSoldOut($order['productid'])) {
				 
				 
				 logic('product')->Update($order['productid'], array('overtime' => time(),
				 													 'status' => PRO_STA_Finish,
				 													 'yungou' => YUNGOU_STA_Readied));
		...
		else
		{
			if ($order['_tmpd_']['paid'])
			{
				...
				if ($sourceStatus == 'WAIT_SELLER_SEND_GOODS') {
										if ($order['_tmpd_']['money'] > 0 && $trade['money'] > 0)
					{
												logic('me')->money()->less($trade['price'], $order['userid'], array(
							'name' => __('无效订单处理'),
				...
				if(logic('pay')->Process($payment, $sourceStatus)) {
			return;
		}
		
		$this->message && $this->master->Messager($error?$error:__('订单已经提交，请等待结果揭晓！'), '?mod=me&code=order');
...
class Recharge_Parser_CallbackLogic extends CallbackLogic
{
	...
	function Parse_TRADE_FINISHED($payment = false)
	{
				$trade = logic('pay')->TradeData($payment);
		if($trade['sign']) {
			$makeRet = logic('recharge')->MakeSuccessed($trade['sign']);
		}		
		if(logic('pay')->Process($payment, 'TRADE_FINISHED')) return;
