...
class MeLogic_Money
{
    ...
    public function seller_money_add($moves, $uid = 0, $log = array()){
        $uid = (int) (($uid > 0) ? $uid : user()->get('id'));
        $moves = doubleval($moves);
        if($uid > 0 && $moves >= 0) {
            $money=floatval(logic('me')->money()->count($uid));

            $sql ='UPDATE ' . table('seller').' SET money = money + ' . $moves . ' , account_money = account_money + ' . $moves . ' WHERE userid = ' . $uid;
            $query = dbc(DBCMax)->query($sql)->done();
	...
	public function logCreate( $uid, $type, $moves, $log, $sourcemoney = 0 )
	{
		...
		else
		{
			...
			$data['class'] = 'sys';
                ...
                if($data['name'] == '商家收款'){
            $data['intro'] .= '<br/>商家收款的金额已经累计到您的销售账户中，如需提出该部分金额，请到商家管理进行“申请结算”';
        }else{
