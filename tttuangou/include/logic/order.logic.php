...
class OrderLogic
{
	...
	public function GetList( $uid = 0, $status = ORD_STA_ANY, $pay = ORD_PAID_ANY, $extend = '1',$notpage = 1,$where='' )
	{
    ...
    public function autoConfirm(){
    	$autoConfirmDay = 7 ;
    	if($autoConfirmDay > 0) {
	        	        $process='WAIT_BUYER_CONFIRM_GOODS';
	        $where = 'pay=1 and process="'.$process.'"';
	        $confirm_time = time()-(86400 * $autoConfirmDay);
	        $where .= ' and process_time<"'.$confirm_time.'"';
	        $orders = dbc(DBCMax)->select('order')->where($where)->done();
	        if($orders){
	            foreach($orders as $val){
	            	$ups2TRADE_FINISHED = false;
	                if ($val['process'] == 'WAIT_BUYER_CONFIRM_GOODS')
	                {
	                    	                    	                    	                    	                    	                    $product = dbc(DBCMax)->select('product')->where('id='.$val['productid'])->limit(1)->done();
	                    $product['type'] == 'stuff' && $ups2TRADE_FINISHED = true;
	                }
	                	                	                if ($ups2TRADE_FINISHED)
	                {
	                    logic('order')->Processed($val['orderid'], 'TRADE_FINISHED');
	                    logic('order')->clog($val['orderid'])->sys('系统内部交易处理变更：' . $autoConfirmDay . ' 天自动确认收货，交易已完成', 'TRADE_FINISHED');

	                    	                    logic('rebate')->Add_Rebate_For_Item($val);
	                    	                }
	            }
	        }
	    }
	...
	public function getAllOrdersByOrderid($orderid)
	{
	    ...
	    $ids = array();
	    foreach($orders as $val) {
	    	$ids[] = (is_array($val) ? $val['orderid'] : $val);
	    }
	    return $this->Where("orderid in(".implode(',', $ids).")");
	}
    ...
    public function is_creditbuy_order($orderid) {
        $order = logic('credit')->GetOne($orderid);
        if(empty($order)) {
            return false;
        }else{
            return true;
    ...
    public function Count_pro($where)
    {
        $sql = 'SELECT SUM(productnum) AS CNT FROM '.table('order').' WHERE '.$where;
        $result = dbc()->Query($sql)->GetRow();
        return $result['CNT'];
    }
	...
	public function _TMP_Payed($trade, $payment = false)
	{
		...
		if ($order['pay'] == ORD_PAID_No)
		{
            ...
            if($order['productid'] != 99999){
                $product = logic('product')->BuysCheck($order['productid'], false, $order['productnum'], ORD_PAID_No, $order['userid']);
            }
            $error = false;
			if (isset($product['false'])) {
				$error = $product['false'];
								...
								if ($product['yungou'] != 0) {
					if ( !in_array($order['paymentinfo']['code'], handler('config')->get('yungou','paycods')) ) {
						$error = __('云购产品不支持“' . $order['paymentinfo']['name'] . '”支付方式！');
					}
				}
						...
						if (!$trade['nmpay'])
			{
				...
				if(($price_cut=logic('cut')->order_calc($order['orderid'], $price, $order['productnum']))>0) {
					$cutss .= sprintf(__('优惠立减：%.2f<br>'), $price_cut);
				}
				if(($promo_cut_price=logic('promo_code')->order_calc($order, $price))>0) {
					$cutss .= sprintf(__('优惠码立减：%.2f<br>'), $promo_cut_price);
				}
				if ($error)
				{
				...
				else
				{
					...
					elseif ($order['status'] != ORD_STA_Normal && $order['status'] != ORD_STA_Virtual)
					{
					...
					elseif ($money >= $price)
					{
						...
						if ($order['expressprice'] > 0)
						{
							$express = sprintf(__('运费：%.2f<br>'), $order['expressprice']);
                                                ...
                                                if($order['productid'] == 99999 && $order['sid']){
                                                        $seller = logic('seller')->GetOne($order['sid']);
                            #钱的减少
                            logic('me')->money()->pay($price, $order['userid'], array(
                                'name' => __('我要买单支付'),
                                'intro' => sprintf(__('订单号：%s<br/>商家名：%s<br/>消费金额：%.2f<br/>'), $order['orderid'], $seller['sellername'], $order['paymoney'])
                            ));
                                                        logic('rebate')->Add_Rebate_For_Paybill($order['sid'],$order);
                        }else{
                            #钱的减少
                            logic('me')->money()->pay($price, $order['userid'], array(
                                'name' => __('购买商品'),
						...
						{
                            if($order['productid'] == 99999 && $order['sid']){
                                $this->Processed($order['orderid'], 'TRADE_FINISHED');
                            }else{
    ...
    public function Credit_TMP_Payed($trade){
        if($trade){
            #钱的减少
            logic('me')->money()->pay($trade['price'], $trade['userid'], array(
                'name' => __('购买积分'),
                'intro' => sprintf(__('订单号：%s<br/>商品名：%s<br/>消费金额：%.2f<br/>'), $trade['sign'], '购买积分', $trade['price'])
            ));
                                                $credit = ini('credit');
            $score = $trade['price'] / $credit['buy_money'];
            logic('credit')->add_score(0,$trade['userid'],$score,'credit_buy','','',$trade['price']);
                        $order_update = array(
                'pay' => ORD_PAID_Yes,
                'status' => 1,
                'paytime' => time()
            );
            return logic('credit')->Update($trade['sign'], $order_update);
        }
    }
	...
	public function ProcessedVirtual($orderid,$process)
	{
	    ...
	    {
	        $this->_Processed((is_array($orderid) ? $orderid['orderid'] : $orderid), $process);
	    }
	    $this->_Processed($orderid, $process);
	}
	...
	public function MakeSuccessed($orderid, $ignore_rules = false)
	{
		...
		$product = logic('product')->BuysCheck($order['productid'], false, $order['productnum'], ORD_PAID_Yes, $order['userid']);
		if (isset($product['false']))
		{
		...
		if (false != meta($MID) || (($order['paytime'] + 3600) < time())) 		{
									return $order;
		}
		$notify_data = array(
	...
	private function __parse_result($data)
	{
		...
		$data['paymentinfo'] = $data['paytype'] > 0 ? logic('pay')->GetOne($data['paytype']) : array('name'=>'-----');
				...
				$data['promo_cut_price'] = logic('promo_code')->order_calc($data, $data['paymoney']);
	...
	function UpOrder()
	{
		$sql = 'update ' . TABLE_PREFIX . 'tttuangou_order o left join ' . TABLE_PREFIX . 'tttuangou_product p on o.productid = p.id set o.status=2 where (' . time() . ' > p.overtime and p.overtime > 0) and  o.pay = 0 and o.status = 1 and o.userid = ' . MEMBER_ID;
		$query = $this->DatabaseHandler->Query($sql);
		return true;
	}
	...
	public function updateOrderUnderProduct($pid, $data) {

		$whereSql = ' productid = ' . $pid;

		return $this->baseUpdate($data, $whereSql);

	}

	
	public function baseUpdate($data, $whereSql) {

		dbc()->SetTable(table('order'));

		return dbc()->Update($data, $whereSql);
    ...
    function _Checkout_save_V($ordersId)
    {
        ...
        {
            $orders[] = $orders_tmp = logic('order')->GetOne($v['orderid']);
        ...
        #生成空白订单
        $order_tmp = logic('order')->__CreateNew(user()->get('id'), $order['productid'], ORD_STA_Virtual);
