...
class CityManageLogic
{
	...
	public function place_navigate($flag = 'product', $tpl_prefix = '')
	{
		if ($this->place_enabled())
		{
			...
			include handler('template')->file(($tpl_prefix . 'city_place_navigate'));
		}
	}
	
	public function place_inputer($product_id, $is_city=false)
	{
		$places = array();
		if ($product_id || MEMBER_ROLE_TYPE == 'seller')
		{
			if(false == $is_city) {
				if($product_id){
                    ...
                    $sinfo = dbc(DBCMax)->query('select uid,area from '.table('members')." where uid='".MEMBER_ID."'")->limit(1)->done();
                    $product = array('city' => $sinfo['area']);
                }
			} else {
				if(is_numeric($product_id)) {
					$product['city'] = $product_id;
    ...
    public function seller_mplace_inputer($seller_id,$region=0)
    {
        $places = array();
        if ($seller_id || MEMBER_ROLE_TYPE == 'seller' || 1 == MEMBER_ID)
        {
            if($seller_id){
                $seller = dbc(DBCMax)->select('members')->where(array('uid' => $seller_id))->limit(1)->done();
                            }elseif(MEMBER_ROLE_TYPE == 'seller'){
                $seller = dbc(DBCMax)->query('select id,area from '.table('seller')." where userid='".MEMBER_ID."'")->limit(1)->done();
            }
            $places = $this->get_places($seller['area']);
            $places = $this->make_selected($places, array('region:'.$seller['city_place_region']));
        }
        include handler('template')->file('@admin/city_mplace_inputer');
    }

	
	public function sub_place_inputer($pid) {
		$pid = (int) $pid;

		include handler('template')->file('@default/city_place_inputer.sub');
	}
    public function home_place_inputer() {

        include handler('template')->file('@default/city_place_inputer');
    }
	...
	public function hot($flag = 'product')
	{
		...
		$cityID = logic('misc')->City('id');
		$ckey = 'city_place/hot-' . "{$flag}-" . $cityID;
		if(false === ($class = fcache($ckey, 900))) {
			...
			if($hs) {
				...
				foreach($class as $k=>$row) {
					if('street' == $row['type']) {
						$row['url'] = logic('url')->create('hot-' . $flag, array('region' => $row['parent_id'], 'street' => $row['id']));
					} else {
						$row['url'] = logic('url')->create('hot-' . $flag, array('region' => $row['id'], 'street' => null));
					}
					$class[$k] = $row;
				}
			}
