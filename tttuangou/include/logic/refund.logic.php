...
class RefundLogic
{
	public function GetList($process = 0, $orderid = '')
	{
										if ($process > 0) {
			...
			$sql_where = '1';
						        ...
						        if($orderid){
                        $sql_where .= ' and orderid in('.$orderid.')';
        }
	...
	public function save($orderid, $money, $reason ,$uid=0, $cash_type = '', $cash_data = array())
	{
		...
		if($order['product']['yungou'] || '99999' == $order['productid'] || meta('p_nar_'.$order['productid'])) {
			return -4;
		}

		logic('cut')->order_calc($order['orderid'], $order['totalprice']);

        		$promo_cut_price = logic('promo_code')->order_calc($order, $order['totalprice']);
