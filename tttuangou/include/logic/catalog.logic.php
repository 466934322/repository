...
class CatalogLogic
{
    ...
    public function goods_Search($where, $limit = 1)
    {
        $dbo = dbc(DBCMax)->select('goods_catalog')->where($where);
        $limit && $dbo->limit($limit);
        return $dbo->done();
    }
    ...
    public function Goodscate_delete($id)
    {
                $catalog = $this->goods_Search('id='.$id);
        if (!$catalog) return false;
        $master = false;
        if ($catalog['parent'] == 0)
        {
        ...
        $pro_where = 'category = '.$id;
                dbc(DBCMax)->delete('goods_catalog')->where('id='.$id)->done();
	...
	public function hot($limit = 0)
	{
		...
		{
			$class[$i]['icon'] 	= $icon_data[$topclass['id']]['icon'];
			if(empty($class[$i]['icon']) && $topclass['parent']) {
				$class[$i]['icon'] 	= $icon_data[$topclass['parent']]['icon'];
			}
			$class[$i]['script'] = stripcslashes($icon_data[$topclass['id']]['script']);
			$class[$i]['selected'] = $topclass['flag'] == $topcss ? true : false;
			$class[$i]['url'] = logic('url')->create('hot-catalog', array('code' => $topclass['flag']));
