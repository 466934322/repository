...
class CommentManageLogic
{
	...
	public function front_get_comments($product_id, $user_id = null)
	{
		...
		$sql = dbc(DBCMax)->select('comments')->where('product_id='. (int) $product_id.' and (status="approved" or user_id='. (int) $user_id.')')->order('toped.desc')->order('timestamp_update.desc')->sql();
		$sql = page_moyo($sql);
