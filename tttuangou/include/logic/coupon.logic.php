...
class CouponLogic
{
	...
	public function GetList( $uid = 0, $oid = 0, $status = TICK_STA_Unused, $pid = false, $spid = '',$where='')
	{
		$sql_limit_user = '1';
		if ($uid > 0)
		{
		...
		$sql = 'SELECT t.*, p.name, p.flag, p.intro, p.perioddate, o.productnum, o.productprice, o.promo_code
		FROM
			' . table('ticket').' t
		LEFT JOIN
			' . table('order').' o
		...
		if($coupons){
			foreach($coupons as $key => $val){
                ...
                if($time_begin && $time_end){
                    $usetime = strtotime($val['usetime']);
                    if(!($usetime>$time_begin && $usetime<$time_end)) {
                        unset($coupons[$key]);
                    }
                }
			}
    ...
    public function GetList_count( $pid = false)
    {
        $sql_limit_product = $pid ? ('productid='.$pid) : '1';
        $sql = 'SELECT SUM(mutis) AS sums
		FROM
			' . table('ticket').'
		WHERE
			'.$sql_limit_product.'
		...
		if ($product)
		{
			switch ($product['coupon']['status'])
			{
				case TICK_STA_Unused :
										if(false == $this->ops_check($product['id'], $ops_member_id))
					{
					...
					else
					{
						...
						else
						{
							...
							$ops_seller = logic('seller')->GetOne(null, $ops_member_id, null);
							foreach($checknumbers as $k => $v){
								$allnumbers[] = $v['number'];
								$product['coupon'] = $v;
																dbc(DBCMax)->update('ticket')->data(array('status' => TICK_STA_Used, 'usetime' => $usetime, 'ops_uid'=>$ops_member_id, 'ops_sname'=>$ops_seller['sellername'], 'ops_sid'=>$ops_seller['id']))->where(array('ticketid' => $product['coupon']['ticketid']))->done();
								...
								$product['coupon']['flag'] = $product['flag'];
																logic('rebate')->Add_Rebate_For_Ticket($product,$ops_member_id);
															}
	...
	public function ops_check($pid, $ops_uid = MEMBER_ID) {
		$pid = (int) $pid;
		$ops_uid = (int) $ops_uid;
		if($pid < 1 || $ops_uid < 1) {
			return false;
		...
		$ops_sid = logic('seller')->U2SID($ops_uid);
		if($ops_sid < 1) {
			return false;
		}
		...
		if($product['sellerid'] == $ops_sid || in_array($ops_sid, $sids)) {
			return true;
		}
