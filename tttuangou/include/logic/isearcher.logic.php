...
class iSearcherLogic
{
	...
	public function inputer($tpl_prefix = '')
	{
		$kw = $this->get_kw();
		include handler('template')->file($tpl_prefix . 'isearcher_inputer');
        return $kw;
	}
	...
	private function _lnk_of_seller_product(&$sql, $key, $wd, $map)
    {
		...
		private function _lnk_of_seller_manage(&$sql, $key, $wd, $map) {
		$this->iiSearch($map, $key, $wd);
        switch ($key)
        {
            case 'pid':
