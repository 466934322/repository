...
class AccountLogic
{
	...
	public function password($password, $mixed = '') {
		if($mixed) {
			if(is_string($mixed)) {
				...
				if($uid > 0) {
										if(false !== strpos($type, '.qq.') || false !== strpos($type, '.wx.')) {
			...
			if ('qq' == $type || false !== strpos($type, '.qq.') || 'wx' == $type || false !== strpos($type, '.wx.')) {
	...
	public function Login($username, $password, $keepLogin = true)
	{
		if('' == trim($username)) {
			...
			if (!empty($exLogin['error']))
			{
			return $this->ErrorInf($exLogin['result']);
		}
			$extend = (empty($exLogin['result']) ? null : $exLogin['result']);
		...
		$this->setLoginStatus($UserFields, $keepLogin);

				return $this->SuccInf($extend);
	}
		public function setLoginStatus($UserFields, $keepLogin = true) {
		...
		if(empty($UserFields['secques'])) { 			$UserFields['secques'] = random(24);
			dbc()->Query("UPDATE " . table('members') . " SET `secques`='{$UserFields['secques']}' WHERE `uid`='{$UserFields['uid']}'");
		}
		$authcode = authcode("{$UserFields['password']}\t{$UserFields['secques']}\t{$UserFields['uid']}\t".time(), 'ENCODE');
		if ( $keepLogin )
		{
...
class AccountLogic_uLogin
{
	...
	public function wxopenid(){
		driver('ulogin')->api('wx')->get_openid();
	}
	...
	public function wx_reg_and_login($from='wx') {

		$ret = false;

		$uInfo = driver('ulogin')->api('wx')->get_user_info();
			$wxid = $uInfo['unionid'];
		if (empty($wxid)) {
			die('用户 wxid 为空！');
			return $ret;
		...
		$unique_key = 'ul.wx.'. $wxid;
		$unique_val = meta($unique_key);
		if (!$unique_val) {
			if (ENC_IS_GBK) {
				$userinfo = array('nickname' => ENC_U2G($uInfo['nickname']));
			}
			else {
				$userinfo = array('nickname' => $uInfo['nickname']);
			}

			$sopassword = random(18);
			$pwd_str = logic('acl') -> licEncrypt($sopassword, 'ENCODE');

					$uNameSearchs = array($userinfo['nickname'], $userinfo['nickname'].'.wechat', $userinfo['nickname'].'.weixin', $wxid.'.wechat');
			$foundName = $this->find_username_in_list($uNameSearchs);
			$userinfo['nickname'] = $foundName ? $foundName : account()->GetFreename();
			...
			empty($userinfo['nickname']) && $userinfo['nickname'] = account()->GetFreename(); 			$email = $wxid .'@wechat.com';
			$result= account()-> RegisterLocal($userinfo['nickname'], $password, $email);
			...
			if (intval($result['result']) > 0) {
				dbc(DBCMax)->update('members')->data($aData)->where('uid = '. $result['result'])->done();
				$rs = meta($unique_key, $wxid ."\n". $result['result']."\n". $userinfo['nickname']."\n". $pwd_str."\n". 1);
			}
			else {
				$nickname = $userinfo['nickname'] = account()->GetFreename(); 				$result = account()->RegisterLocal($nickname, $password, $email);
				dbc(DBCMax)->update('members')->data($aData)->where('uid = '.$result['result'])->done();
				$rs = meta($unique_key, $wxid."\n". $result['result']."\n". $nickname ."\n". $pwd_str."\n". 1);
					...
					$this->is_first_login = $unique_key;

					$loginR = account()-> Login($userinfo['nickname'], $password);
			if (($result['result'] > 0) && ($rs > 0) && ($loginR['error'] === false)) {
				$ret = true;
			}
			else {
				$ret = false;
	...
	public function wxrebind($p = array(), $dorebind = false) {
		$p = ($p ? $p : $_POST);

				$nuid = trim($p['nuid']);
		if('' == $nuid) {
			return 'nuid为空，您不能执行此绑定操作！';
		}
		$meta = explode("\n", meta($nuid));
		if(false == $meta) {
			return 'nuid错误，您不能执行此绑定操作！';
		}
		if(false == $meta[4]) {
			return '您不能再执行绑定操作了！';
		}
		if($meta[0] != substr($nuid, 6) || false == ($user = user($meta[1])->get()) || MEMBER_ID != $user['id'] || $nuid != $user['email2']) {
			return '您不能执行此绑定操作！';
		}
		$act = ('reg' == $p['act'] ? 'reg' : 'login');
		...
		if('' == $username || '' == $password) {
			return '用户名或密码不能为空！';
				...
				if('login' == $act) {
						$check = handler('member')->CheckMember($username, $password);
			if ($check == -1) {
				return '用户名或密码错误，您可以有最多 5 次尝试！';
			} elseif ($check == 0) {
				return '用户名不存在，您可以有最多 5 次尝试！';
			}

						$UserFields = handler('member')->GetMemberFields();
			if('' != $UserFields['email2']) {
				return '该用户是第三方登录账户，不能执行绑定操作！';
			}

			if($dorebind) {
				$uid = $UserFields['uid'];
				dbc(DBCMax)->update('members')->data(array('email2'=>$nuid, 'password'=>md5(md5($password)), 'secques'=>random(24)))->where(array('uid'=>$UserFields['uid']))->done();
							}

				} else {
			$password2 = trim($p['password2']);
			...
			if('' == $mail) {
				return 'Email 地址不能为空！';
						...
						if (false != ($ret = account()->invaidAccount($username, $password, $mail))) {
				return $ret;
			}
						if (logic('account')->Exists('name', $username)) {
				return '用户名已经存在！';
			...
			if (logic('account')->Exists('mail', $mail)) {
				return 'Email 地址已经被使用！';
			}

			if($dorebind) {
				$uid = $user['id'];
				dbc(DBCMax)->update('members')->data(array('username'=>$username, 'password'=>md5(md5($password)), 'email'=>$mail, 'secques'=>random(24)))->where(array('uid'=>$user['id']))->done();
			}
		}

		if($dorebind && $uid > 0) {
			$pwd_str = logic('acl') -> licEncrypt($password, 'ENCODE');
			meta($nuid, "{$rs[0]}\n{$uid}\n{$username}\n{$pwd_str}");
			account()->Login($username, md5($password));
		}
	}
...
class AccountLogic_ex_UCenter
{
	...
	public function Login($username, $password)
	{
				...
				if ($uc_uid > 0 && $locUser && md5($uc_password) != $locUser['password'])
		{
				...
				if($uc_uid > 0 && $locUser['id'] > 0 && $locUser['ucuid'] != $uc_uid) {
