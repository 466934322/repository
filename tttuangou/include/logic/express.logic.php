...
class ExpressLogic
{
	...
	function html( $data )
	{
		switch (mocod())
		{
			case 'buy.checkout':
			    ...
			    if (logic('buy_type')->CheckStuff($data) === false) {
					return;
				}
	...
	function GetList($aid = 0, $pid = false, $appuid = 0)
	{
		...
		if ($pid && meta('expresslist_of_'.$pid))
		{
			...
			foreach($return as $i=>$one) {
				$cIndex = $one['firstunit'].'-'.$one['continueunit'].'-'.$one['firstprice'].'-'.$one['continueprice'].'-'.$one['fp'].'-'.$one['cp'];
				$cHash = md5($cIndex);
	...
	function Accessed($class, &$data)
	{
		if ($class == 'order.save')
		{
			...
			$product = logic('product')->BuysCheck($data['productid'], true, false, null, $data['userid']);
		...
		if(is_array($orders) && count($orders) > 1 && $vorder['expresstype']) {
			...
			foreach($orders as $order) {
				if($order['expresstype'] == $vorder['expresstype'] && $vorder['addressid'] == $order['addressid']) {
					$expresscount++;
					$product = logic('product')->BuysCheck($order['productid'], true, false, null, $order['userid']);
