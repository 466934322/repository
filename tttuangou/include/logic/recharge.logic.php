...
class card_RechargeLogic
{
	...
	public function MakeUsed($number, $password, $uid = null)
	{
		$uid = (null === $uid ? user()->get('id') : (int) $uid);
		return dbc(DBCMax)->update('recharge_card')->data(array('usetime'=>time(),'uid'=>$uid))->where(array('number'=>$number,'password'=>$password))->done();
	}
	...
	{
		$price = min(99999.99, max(0, round((float) $price, 2)));
		$nums = min(9999, max(0, (int) $nums));
