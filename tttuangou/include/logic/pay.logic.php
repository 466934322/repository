...
class PayLogic
{
	...
	public function html($data, $tpl_suffix = '')
	{
		switch (mocod())
		{
			case 'buy.order':
				...
				$isYunGou = $data['product']['yungou'] > 0 ? true : false;						include handler('template')->file('@html/pay_selector' . $tpl_suffix);
				break;
            case 'buy.Order':
                $pay_money = $data['price_of_total'];
                $product_id = $data['productid'];
                include handler('template')->file('@html/pay_selector' . $tpl_suffix);
                break;
			case 'recharge.order':
