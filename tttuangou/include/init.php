...
class TTTGMaster
{
	public static function load($file)
	{
		define('ROOT_PATH', substr(dirname(__FILE__), 0, -8) . '/'); 
				...
				define('DEBUG', is_file(ROOT_PATH . 'data/debug.'.WEB_BASE_ENV_DFS::$APPNAME.'.signal') ? true : false);
				$iniz = new initialize();
		$iniz->envInit();
...
class initialize
{
	...
	function initialize()
	{
		require_once ROOT_PATH . 'setting/settings.php';
		$this->config = $config['settings'];
				if ($this->config['site_domain'] != $_SERVER['HTTP_HOST'] && false == in_array($this->config['site_domain'], array('localx.uuland.org', 'dev.tttuangou.net', 'tuangou.com', )))
		{
	...
	{
		$config = $this->config;
				if (in_array($module, array('', 'index')) && false == in_array($_GET['mod'], array('api', 'callback'))) {
