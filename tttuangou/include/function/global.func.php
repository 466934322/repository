...
function errorlog($logData, $filename = 'errorlog', $halt = false) {
	$logSaveDir = ROOT_PATH . 'cache/errorlog/' . date('m') . '/'; tmkdir($logSaveDir);
	
	$filename .= '.' . substr(md5($logSaveDir . $filename), -6) . '.' . date('md') . '.log';

	if(is_array($logData)) {
		$logData = var_export($logData, true);
	}

	$logData = "[{$_SERVER['REQUEST_METHOD']} . " . date('Y-m-d H:i:s') . " . " . client_ip() . "]\r\n{$logData}\r\n\$_REQUEST : " . var_export($_REQUEST, true) . "\r\n\r\n";

	error_log($logData, 3, $logSaveDir . $filename);

	$halt && exit;
}


function derror($data='ERROR', $code=-1) {
	errorlog($data, 'derror');
	return dresult($data, $code, true);
}
function dresult($data='OK', $code=0, $error=false) {
	return array(
			'error'=>$error,
			'code'=>$code,
			'result'=>$data,			
...
function dresponse($result, $halt=true) {
	$output = json_encode(((is_array($result) && isset($result['error']) && isset($result['code']) && isset($result['result'])) ? $result : dresult($result)));
