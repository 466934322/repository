...
function page_html_ajax($onclick_callback = '') {
	$ps = page_moyo_summary();
	...
	$pfirst = '<a href="javascript:void()" onclick="'.sprintf($onclick_callback, 1).';return false;" style="margin-left:0;"><font class="page_first">首页</font></a>';
	$plast = '<a href="javascript:void()" onclick="'.sprintf($onclick_callback, $pa).';return false;"><font class="page_last">尾页</font></a>';
		$plist = '';
	$al = 4;
	if ($pn - $al/2 <= 0) $pfrom = 1;
	else $pfrom = $pn - $al/2 + 1;
	...
	for ($pi = $pfrom; $pi < $pend; $pi++)
	{
		if ($pi > $pa) break;
		if ($pi != $pn)
		$plist .= '<a href="javascript:void()" onclick="'.sprintf($onclick_callback, $pi).';return false;"><font class="page_number">'.$pi.'</font></a>';
		else $plist .= '<font class="page_current">'.$pi.'</font> ';
	}
	$html = $pfirst.''.$pre.''.$plist.''.$nxt.''.$plast;
	$html .= '<div class="page_count">共 '.$total.' 条记录</div>';
...
{
	$backup = $value;
		$value = array_iconv('GBK', 'UTF-8', $value);
	if (empty($value))
	{
		$value = $backup;
	}
...
{
	$backup = $value;
		$value = array_iconv('UTF-8', 'GBK', $value);
	if (empty($value))
	{
		$value = $backup;
	}
...
function api_uid($appcode = '', $token = '', $apptoken = '') {
	...
	if($uid < 1) {
		errorlog('api_uid() is empty', 'extend.api_uid');
	}
	return $uid;
}

function is_client_request() {
	return ((get('appcode') && get('token')) || false != handler('cookie')->GetVar('apptoken'));
}
