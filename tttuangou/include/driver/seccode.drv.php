 ...
 * @package driver
 * @name seccode.drv.php
 * @version 1.0
 */

function tmt_rand($m, $n) {	
	if($m > $n) {
		return mt_rand($n, $m);
	} else {
		return mt_rand($m, $n);
...
class SeccodeDriver {
	...
	var $width 	= 150;
	var $height 	= 60;
	var $background	= 1; 	var $adulterate	= 0; 	var $ttf 	= 0; 	var $angle 	= 1; 	var $warping 	= 0; 	var $scatter	= 0; 	var $color 	= 1; 	var $size 	= 1; 	var $shadow 	= 1; 	var $animator 	= 0; 	var $fontpath	= '';
	var $datapath	= '';
	var $includepath= '';
	...
	function background() {
		...
		if($this->background && function_exists('imagecreatefromjpeg') && function_exists('imagecolorat') && function_exists('imagecopymerge') &&
			...
			$cacheid = 'seccode/backgrounds';
			if(false === ($backgrounds = fcache($cacheid, -1))) {
				$backgrounds = array();
			...
			if($backgrounds) {
				...
				imagecopymerge($this->im, $imwm, 0, 0, tmt_rand(0, 200 - $this->width), tmt_rand(0, 80 - $this->height), imageSX($imwm), imageSY($imwm), 100);
				imagedestroy($imwm);
			}
		}
		if(!$this->background || !$backgrounds) {
			for($i = 0;$i < 3;$i++) {
				$start[$i] = tmt_rand(200, 255);$end[$i] = tmt_rand(100, 150);$step[$i] = ($end[$i] - $start[$i]) / $this->width;$c[$i] = $start[$i];
			}
	...
	function adulterate() {
		...
		for($i = 0; $i <= $linenums;$i++) {
			$color = $this->color ? imagecolorallocate($this->im, tmt_rand(0, 255), tmt_rand(0, 255), tmt_rand(0, 255)) : imagecolorallocate($this->im, $this->fontcolor[0], $this->fontcolor[1], $this->fontcolor[2]);
			$x = tmt_rand(0, $this->width);
			$y = tmt_rand(0, $this->height);
			if(tmt_rand(0, 1)) {
				$w = tmt_rand(0, $this->width);
				$h = tmt_rand(0, $this->height);
				$s = tmt_rand(0, 360);
				$e = tmt_rand(0, 360);
				for($j = 0;$j < 3;$j++) {
				...
				$xe = tmt_rand(0, $this->width);
				$ye = tmt_rand(0, $this->height);
	...
	function adulteratefont() {
		...
		for($i = 0; $i <= 3; $i++) {
			$adulteratecode = $seccodeunits[tmt_rand(0, 23)];
			imagechar($this->im, 5, $x * $i + tmt_rand(0, $x - 10), tmt_rand($y, $this->height - 10 - $y), $adulteratecode, $text_color);
		}
	}
	...
	function ttffont() {
		...
		$cacheid = 'seccode/ttffont-seccodettf_'.($this->type?'ch':'en');
		if(false === ($seccodettf = fcache($cacheid, -1))) {
			$dirs = opendir($seccoderoot);
			$seccodettf = array();
			while($entry = readdir($dirs)) {
				if($entry != '.' && $entry != '..' && in_array(strtolower(fileext($entry)), array('ttf', 'ttc'))) {
					if(false == in_array($entry, array('Banana Chip Mufflins.ttf', 'Bushido.ttf', 'fck me harder.ttf', 'Zepto 1.000.ttf', ))) {
						$seccodettf[] = $entry;
					}
				}
			}
			fcache($cacheid, $seccodettf);
		}
		if(empty($seccodettf)) {
			$this->giffont();
			return;
		...
		for($i = 0; $i < $seccodelength; $i++) {
			...
			$font[$i]['angle'] = $this->angle ? tmt_rand(-30, 30) : 0;
			$font[$i]['size'] = $this->type ? $this->width / 7 : $this->width / 6;
			$this->size && $font[$i]['size'] = tmt_rand($font[$i]['size'] - $this->width / 40, $font[$i]['size'] + $this->width / 20);
			...
			$font[$i]['width'] = (max($box[2], $box[4]) - min($box[0], $box[6])) + tmt_rand(0, $this->width / 8);
			$font[$i]['width'] = $font[$i]['width'] > $this->width / $seccodelength ? $this->width / $seccodelength : $font[$i]['width'];
			$widthtotal += $font[$i]['width'];
		}
		$x = tmt_rand($font[0]['angle'] > 0 ? cos(deg2rad(90 - $font[0]['angle'])) * $font[0]['zheight'] : 1, $this->width - $widthtotal);
		!$this->color && $text_color = imagecolorallocate($this->im, $this->fontcolor[0], $this->fontcolor[1], $this->fontcolor[2]);
		for($i = 0; $i < $seccodelength; $i++) {
			if($this->color) {
				$this->fontcolor = array(tmt_rand(0, 255), tmt_rand(0, 255), tmt_rand(0, 255));
				$this->shadow && $text_shadowcolor = imagecolorallocate($this->im, 0, 0, 0);
				$text_color = imagecolorallocate($this->im, $this->fontcolor[0], $this->fontcolor[1], $this->fontcolor[2]);
			} elseif($this->shadow) {
				$text_shadowcolor = imagecolorallocate($this->im, 0, 0, 0);
			}
			$y = $font[0]['angle'] > 0 ? tmt_rand($font[$i]['height'], $this->height) : tmt_rand($font[$i]['height'] - $font[$i]['hd'], $this->height - $font[$i]['hd']);
	...
	function giffont() {
		$seccode = $this->code;
		$cacheid = 'seccode/giffont-seccodedir';
		if(false === ($seccodedir = fcache($cacheid, -1))) {
			$seccodedir = array();
		...
		for($i = 0; $i <= 3; $i++) {
			...
			if(!empty($this->imcodefile) && file_exists($this->imcodefile)) {
				...
				$font[$i]['width'] = $font[$i]['data'][0] + tmt_rand(0, 6) - 4;
				$font[$i]['height'] = $font[$i]['data'][1] + tmt_rand(0, 6) - 4;
				$font[$i]['width'] += tmt_rand(0, $this->width / 5 - $font[$i]['width']);
				$widthtotal += $font[$i]['width'];
			} else {
				$font[$i]['file'] = '';
				$font[$i]['width'] = 8 + tmt_rand(0, $this->width / 5 - 5);
				$widthtotal += $font[$i]['width'];
			}
		}
		$x = tmt_rand(1, $this->width - $widthtotal);
		for($i = 0; $i <= 3; $i++) {
			$this->color && $this->fontcolor = array(tmt_rand(0, 255), tmt_rand(0, 255), tmt_rand(0, 255));
			if($font[$i]['file']) {
				$this->imcode = imagecreatefromgif($font[$i]['file']);
				if($this->size) {
					$font[$i]['width'] = tmt_rand($font[$i]['width'] - $this->width / 20, $font[$i]['width'] + $this->width / 20);
					$font[$i]['height'] = tmt_rand($font[$i]['height'] - $this->width / 20, $font[$i]['height'] + $this->width / 20);
				}
				$y = tmt_rand(0, $this->height - $font[$i]['height']);
				if($this->shadow) {
	...
	function flash() {
		...
		$fontcolor = '0x'.(sprintf('%02s', dechex (tmt_rand(0, 255)))).(sprintf('%02s', dechex (tmt_rand(0, 128)))).(sprintf('%02s', dechex (tmt_rand(0, 255))));
		$strAction = "
	...
	function bitmap() {
		...
		foreach($numbers as $i => $number) {
			for($j = 0; $j < 6; $j++) {
				$a1 = substr('012', tmt_rand(0, 2), 1).substr('012345', tmt_rand(0, 5), 1);
				$a2 = substr('012345', tmt_rand(0, 5), 1).substr('0123', tmt_rand(0, 3), 1);
				tmt_rand(0, 1) == 1 ? array_push($numbers[$i], $a1) : array_unshift($numbers[$i], $a1);
				tmt_rand(0, 1) == 0 ? array_push($numbers[$i], $a1) : array_unshift($numbers[$i], $a2);
			}
		}
		...
		for($i = 0; $i < 8; $i++) {
			$a = substr('012345', tmt_rand(0, 2), 1) . substr('012345', tmt_rand(0, 5), 1);
